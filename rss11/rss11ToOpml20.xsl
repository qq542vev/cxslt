...
<xsl:stylesheet
	version="1.0"
	id="cxsltl.rss11ToOpml20.stylesheet"
...
>
	...
	<xsltdoc:XSLT10Stylesheet rdf:about="">
		<xsltdoc:short xml:lang="en">XSLT Stylesheet which converts RSS 1.1 format into OPML 2.0 format.</xsltdoc:short>
		<xsltdoc:short xml:lang="ja">RSS 1.1 形式で書かれたファイルを OPML 2.0 形式に変換する XSLT です。</xsltdoc:short>
	...
	<xsl:param name="cxsltl:rss11ToOpml20.deny" select="/.."/>
	...
	<xsltdoc:RuledTemplate>
		<xsltdoc:short>ルートノード以下のRSS 1.1の要素をOPML 2.0に変換する</xsltdoc:short>
		<xsltdoc:detail>
			このXSLTでXMLを変換した場合、最初に呼び出される。
			アクセス修飾子はpublicとなっているが、xsl:apply-templatesからこのテンプレートを適用すべきではない。
			OPML 2.0への変換はxsl:call-templateを使用しcxsltl:rss11ToOpml20.convertを呼び出すべきである。
		</xsltdoc:detail>
	...
	<xsl:template match="/">
		<xsl:call-template name="cxsltl:rss11ToOpml20.convert">
	...
	<xsl:template match="/ | *" mode="cxsltl:rss11ToOpml20.converter">
		<xsl:param name="deny" select="$cxsltl:rss11ToOpml20.deny"/>
		<xsl:param name="denyCount" select="count($deny)"/>

		<xsl:apply-templates select="(node() | @*)[count(. | $deny) != $denyCount]" mode="cxsltl:rss11ToOpml20.converter">
	...
	<xsl:template match="text() | @*" mode="cxsltl:rss11ToOpml20.converter"/>
	...
	<xsl:template match="rss:Channel" mode="cxsltl:rss11ToOpml20.converter">
		<xsl:param name="deny" select="$cxsltl:rss11ToOpml20.deny"/>
		<xsl:param name="denyCount" select="count($deny)"/>

		<opml version="2.0">
			<head>
				<xsl:apply-templates select="*[not(self::rss:items)][count(. | $deny) != $denyCount]" mode="cxsltl:rss11ToOpml20.converter">
					<xsl:with-param name="deny" select="$deny"/>
					<xsl:with-param name="denyCount" select="$denyCount"/>
				</xsl:apply-templates>
			...
			<body>
				<xsl:apply-templates select="rss:items[@rdf:parseType = 'Collection'][count(. | $deny) != $denyCount]" mode="cxsltl:rss11ToOpml20.converter">
					<xsl:with-param name="deny" select="$deny"/>
					<xsl:with-param name="denyCount" select="$denyCount"/>
				</xsl:apply-templates>
			</body>
	...
	<xsl:template match="rss:Channel/rss:title" mode="cxsltl:rss11ToOpml20.converter">
		<title>
	...
	<xsl:template match="rss:Channel/dc:creator" mode="cxsltl:rss11ToOpml20.converter">
		<ownerName>
			<xsl:value-of select="normalize-space()"/>
		</ownerName>
	</xsl:template>
	...
	<xsl:template match="rss:Channel/dc:date" mode="cxsltl:rss11ToOpml20.converter">
		<dateModified>
	...
	<xsl:template match="rss:item" mode="cxsltl:rss11ToOpml20.converter">
		<xsl:param name="deny" select="$cxsltl:rss11ToOpml20.deny"/>
		<xsl:param name="denyCount" select="count($deny)"/>

		<outline>
			<xsl:variable name="subject">
				<xsl:apply-templates select="dc:subject[not(contains(., ','))][count(. | $deny) != $denyCount]" mode="cxsltl:rss11ToOpml20.converter">
					<xsl:with-param name="deny" select="$deny"/>
					<xsl:with-param name="denyCount" select="$denyCount"/>
				</xsl:apply-templates>
			</xsl:variable>
			...
			<xsl:apply-templates select="*[not(self::dc:subject)][count(. | $deny) != $denyCount]" mode="cxsltl:rss11ToOpml20.converter">
				<xsl:with-param name="deny" select="$deny"/>
				<xsl:with-param name="denyCount" select="$denyCount"/>
			</xsl:apply-templates>
		</outline>
	...
	<xsl:template match="rss:item/rss:title" mode="cxsltl:rss11ToOpml20.converter">
	...
	<xsl:template match="rss:item/rss:link" mode="cxsltl:rss11ToOpml20.converter">
		<xsl:param name="deny" select="$cxsltl:rss11ToOpml20.deny"/>
		<xsl:param name="denyCount" select="count($deny)"/>
	...
	<xsl:template match="rss:item/dc:date" mode="cxsltl:rss11ToOpml20.converter">
	...
	<xsl:template match="rss:item/dc:subject[not(contains(., ','))]" mode="cxsltl:rss11ToOpml20.converter">
	...
	<xsltdoc:NamedTemplate>
		<xsltdoc:short>RSS 1.1をOPML 2.0に変換する</xsltdoc:short>
	...
	<xsl:template name="cxsltl:rss11ToOpml20.convert">
