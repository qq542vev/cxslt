...
<xsl:stylesheet
	version="1.0"
	id="cxsltl.rss11ToRss20.stylesheet"
	exclude-result-prefixes="admin ag cxsltl payload rdf rss rss091 sy xsl xsltdoc"
...
>
	...
	<xsltdoc:XSLT10Stylesheet rdf:about="">
		<xsltdoc:short xml:lang="en">XSLT Stylesheet which converts RSS 1.1 format into RSS 2.0 format.</xsltdoc:short>
		<xsltdoc:short xml:lang="ja">RSS 1.1 形式で書かれたファイルを RSS 2.0 形式に変換する XSLT です。</xsltdoc:short>
	...
	<xsl:param name="cxsltl:rss11ToRss20.deny" select="/.."/>
	...
	<xsltdoc:RuledTemplate>
		<xsltdoc:short>ルートノード以下のRSS 1.1の要素をRSS 2.0に変換する</xsltdoc:short>
		<xsltdoc:detail>
			このXSLTでXMLを変換した場合、最初に呼び出される。
			アクセス修飾子はpublicとなっているが、xsl:apply-templatesからこのテンプレートを適用すべきではない。
			RSS 2.0への変換はxsl:call-templateを使用しcxsltl:rss11ToRss20.convertを呼び出すべきである。
		</xsltdoc:detail>
	...
	<xsl:template match="/">
		<rss version="2.0">
			<channel>
				...
				<xsl:call-template name="cxsltl:rss11ToRss20.convert">
	...
	<xsl:template match="/ | *" mode="cxsltl:rss11ToRss20.converter">
		<xsl:param name="deny" select="$cxsltl:rss11ToRss20.deny"/>
		<xsl:param name="denyCount" select="count($deny)"/>

		<xsl:apply-templates select="(node() | @*)[count(. | $deny) != $denyCount]" mode="cxsltl:rss11ToRss20.converter">
	...
	<xsl:template match="text() | @*" mode="cxsltl:rss11ToRss20.converter"/>
	...
	<xsl:template match="rss091:*" mode="cxsltl:rss11ToRss20.converter"/>
	...
	<xsl:template match="rss:Channel" mode="cxsltl:rss11ToRss20.converter">
		<xsl:param name="deny" select="$cxsltl:rss11ToRss20.deny"/>
		<xsl:param name="denyCount" select="count($deny)"/>
		...
		<xsl:apply-templates select="*[not(self::rss:items)][count(. | $deny) != $denyCount]" mode="cxsltl:rss11ToRss20.converter">
			<xsl:with-param name="deny" select="$deny"/>
			<xsl:with-param name="denyCount" select="$denyCount"/>
		</xsl:apply-templates>
		...
		<xsl:apply-templates select="rss:items[@rdf:parseType = 'Collection'][count(. | $deny) != $denyCount]" mode="cxsltl:rss11ToRss20.converter">
			<xsl:with-param name="deny" select="$deny"/>
			<xsl:with-param name="denyCount" select="$denyCount"/>
		</xsl:apply-templates>
	</xsl:template>
	...
	<xsltdoc:RuledTemplate>
		<xsltdoc:short>{rss:Channel/*}要素、{rss:image/*}要素、{rss:item/*}要素をRSS 2.0に変換する</xsltdoc:short>
	...
	" mode="cxsltl:rss11ToRss20.converter" priority="-0.375">
		<xsl:copy>
	...
	" mode="cxsltl:rss11ToRss20.converter" priority="-0.375">
		...
		<xsl:copy>
			<xsl:apply-templates select="@*[count(. | $deny) != $denyCount]" mode="cxsltl:rss11ToRss20.converter">
				<xsl:with-param name="deny" select="$deny"/>
				<xsl:with-param name="denyCount" select="$denyCount"/>
			</xsl:apply-templates>
	...
	<xsl:template match="@rdf:datatype[starts-with(., 'http://purl.org/dc/terms/')][substring-after(., 'http://purl.org/dc/terms/')]" mode="cxsltl:rss11ToRss20.converter">
	...
	<xsl:template match="@rdf:datatype[starts-with(., 'http://www.w3.org/2001/XMLSchema#')][substring-after(., 'http://www.w3.org/2001/XMLSchema#')]" mode="cxsltl:rss11ToRss20.converter">
		<xsl:attribute name="xsi:type">
	...
	<xsl:template match="rss:title | rss:link | rss:description | rss:url" mode="cxsltl:rss11ToRss20.converter">
	...
	<xsltdoc:RuledTemplate>
		<xsltdoc:short>{rss:image[@rdf:parseType = 'Resource']}要素をRSS 2.0に変換する</xsltdoc:short>
		<xsltdoc:paramNodeSet xsltdoc:name="deny" xsltdoc:short="変換を除外するノード"/>
		<xsltdoc:paramNumber xsltdoc:name="denyCount" xsltdoc:short="{$deny}の個数"/>
	...
	<xsl:template match="rss:image[@rdf:parseType = 'Resource']" mode="cxsltl:rss11ToRss20.converter">
		<xsl:param name="deny" select="$cxsltl:rss11ToRss20.deny"/>
		<xsl:param name="denyCount" select="count($deny)"/>

		<image>
			<xsl:if test="
				not(rss:link[count(. | $deny) != $denyCount]) and
				parent::rss:Channel[count(. | $deny) != $denyCount]/rss:link[count(. | $deny) != $denyCount]
			">
				<xsl:apply-templates select="parent::rss:Channel/rss:link" mode="cxsltl:rss11ToRss20.converter">
					<xsl:with-param name="deny" select="$deny"/>
					<xsl:with-param name="denyCount" select="$denyCount"/>
				</xsl:apply-templates>
			</xsl:if>
	...
	<xsl:template match="rss:Channel/admin:errorReportsTo[starts-with(@rdf:resource, 'mailto:')]" mode="cxsltl:rss11ToRss20.converter">
		<xsl:param name="deny" select="$cxsltl:rss11ToRss20.deny"/>
		<xsl:param name="denyCount" select="count($deny)"/>
	...
	<xsl:template match="rss:Channel/dc:date" mode="cxsltl:rss11ToRss20.converter">
		<xsl:param name="deny" select="$cxsltl:rss11ToRss20.deny"/>
		<xsl:param name="denyCount" select="count($deny)"/>
	...
	<xsl:template match="rss:Channel/dc:language" mode="cxsltl:rss11ToRss20.converter">
		<xsl:param name="deny" select="$cxsltl:rss11ToRss20.deny"/>
		<xsl:param name="denyCount" select="count($deny)"/>
	...
	<xsl:template match="rss:Channel/dc:rights" mode="cxsltl:rss11ToRss20.converter">
		<xsl:param name="deny" select="$cxsltl:rss11ToRss20.deny"/>
		<xsl:param name="denyCount" select="count($deny)"/>
	...
	<xsl:template match="rss:Channel/dc:subject | rss:item/dc:subject" mode="cxsltl:rss11ToRss20.converter">
		<category>
			<xsl:value-of select="normalize-space()"/>
		</category>
	</xsl:template>
	...
	<xsl:template match="rss:item" mode="cxsltl:rss11ToRss20.converter">
		<xsl:param name="deny" select="$cxsltl:rss11ToRss20.deny"/>
		<xsl:param name="denyCount" select="count($deny)"/>
	...
	<xsl:template match="rss:item/rss:description" mode="cxsltl:rss11ToRss20.converter">
		<xsl:param name="deny" select="$cxsltl:rss11ToRss20.deny"/>
		<xsl:param name="denyCount" select="count($deny)"/>
	...
	<xsl:template match="rss:item/dc:date" mode="cxsltl:rss11ToRss20.converter">
		<pubDate>
			<xsl:call-template name="cxsltl:date.iso8601.datetime.parse">
				<xsl:with-param name="callback" select="$cxsltl:date.common.self/xsl:template[@name = 'cxsltl:date.common.toEmailDateTime']"/>
			</xsl:call-template>
	...
	<xsl:template match="rss:item/dc:identifier" mode="cxsltl:rss11ToRss20.converter">
		<xsl:param name="deny" select="$cxsltl:rss11ToRss20.deny"/>
		<xsl:param name="denyCount" select="count($deny)"/>
	...
	<xsl:template match="rss:item/payload:payload[@rdf:parseType = 'Literal']" mode="cxsltl:rss11ToRss20.converter">
		<xsl:param name="deny" select="$cxsltl:rss11ToRss20.deny"/>
		<xsl:param name="denyCount" select="count($deny)"/>
	...
	<xsltdoc:NamedTemplate>
		<xsltdoc:short>RSS 1.1をRSS 2.0に変換する</xsltdoc:short>
	...
	<xsl:template name="cxsltl:rss11ToRss20.convert">
