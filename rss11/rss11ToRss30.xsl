...
<xsl:stylesheet
	version="1.0"
	id="cxsltl.rss11ToRss30.stylesheet"
...
>
	...
	<xsltdoc:XSLT10Stylesheet rdf:about="">
		<xsltdoc:short xml:lang="en">XSLT Stylesheet which converts RSS 1.1 format into RSS 3.0 format.</xsltdoc:short>
		<xsltdoc:short xml:lang="ja">RSS 1.1 形式で書かれたファイルを RSS 3.0 形式に変換する XSLT です。</xsltdoc:short>
	...
	<xsl:param name="cxsltl:rss11ToRss30.deny" select="/.."/>
	...
	<xsltdoc:RuledTemplate>
		<xsltdoc:short>ルートノード以下のRSS 1.1の要素をRSS 3.0に変換する</xsltdoc:short>
		<xsltdoc:detail>
			このXSLTでXMLを変換した場合、最初に呼び出される。
			アクセス修飾子はpublicとなっているが、xsl:apply-templatesからこのテンプレートを適用すべきではない。
			RSS 3.0への変換はxsl:call-templateを使用しcxsltl:rss11ToRss30.convertを呼び出すべきである。
		</xsltdoc:detail>
	...
	<xsl:template match="/">
		<xsl:call-template name="cxsltl:rss11ToRss30.convert">
	...
	<xsl:template match="/ | *" mode="cxsltl:rss11ToRss30.converter">
		<xsl:param name="deny" select="$cxsltl:rss11ToRss30.deny"/>
		<xsl:param name="denyCount" select="count($deny)"/>

		<xsl:apply-templates select="(node() | @*)[count(. | $deny) != $denyCount]" mode="cxsltl:rss11ToRss30.converter">
	...
	<xsl:template match="text() | @*" mode="cxsltl:rss11ToRss30.converter"/>
	...
	<xsl:template match="rss:Channel" mode="cxsltl:rss11ToRss30.converter">
		<xsl:param name="deny" select="$cxsltl:rss11ToRss30.deny"/>
		<xsl:param name="denyCount" select="count($deny)"/>

		<xsl:apply-templates select="*[not(self::rss:items)][count(. | $deny) != $denyCount]" mode="cxsltl:rss11ToRss30.converter">
			<xsl:with-param name="deny" select="$deny"/>
			<xsl:with-param name="denyCount" select="$denyCount"/>
		</xsl:apply-templates>

		<xsl:apply-templates select="rss:items[@rdf:parseType = 'Collection'][count(. | $deny) != $denyCount]" mode="cxsltl:rss11ToRss30.converter">
			<xsl:with-param name="deny" select="$deny"/>
			<xsl:with-param name="denyCount" select="$denyCount"/>
		</xsl:apply-templates>
	</xsl:template>
	...
	<xsl:template match="rss:item" mode="cxsltl:rss11ToRss30.converter">
		<xsl:param name="deny" select="$cxsltl:rss11ToRss30.deny"/>
		<xsl:param name="denyCount" select="count($deny)"/>

		<xsl:text>&#xA;</xsl:text>

		<xsl:apply-templates select="*[count(. | $deny) != $denyCount]" mode="cxsltl:rss11ToRss30.converter">
			<xsl:with-param name="deny" select="$deny"/>
			<xsl:with-param name="denyCount" select="$denyCount"/>
		</xsl:apply-templates>
	</xsl:template>
	...
	<xsl:template match="rss:title | rss:description | dc:language | dc:rights | dc:subject" mode="cxsltl:rss11ToRss30.converter">
		<xsl:call-template name="cxsltl:rss30.generate.line"/>
	</xsl:template>

	<xsltdoc:RuledTemplate>
		<xsltdoc:short>{rss:link}要素をRSS 3.0に変換する</xsltdoc:short>
		<xsltdoc:paramNodeSet xsltdoc:name="deny" xsltdoc:short="変換を除外するノード"/>
		<xsltdoc:paramNumber xsltdoc:name="denyCount" xsltdoc:short="{$deny}の個数"/>
	...
	<xsl:template match="rss:link" mode="cxsltl:rss11ToRss30.converter">
		<xsl:param name="deny" select="$cxsltl:rss11ToRss30.deny"/>
		<xsl:param name="denyCount" select="count($deny)"/>
	...
	<xsl:template match="dc:creator" mode="cxsltl:rss11ToRss30.converter">
		<xsl:call-template name="cxsltl:rss30.generate.line">
	...
	<xsl:template match="dc:date" mode="cxsltl:rss11ToRss30.converter">
		<xsl:call-template name="cxsltl:rss30.generate.line">
	...
	<xsltdoc:NamedTemplate>
		<xsltdoc:short>RSS 1.1をRSS 3.0に変換する</xsltdoc:short>
	...
	<xsl:template name="cxsltl:rss11ToRss30.convert">
